<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if(!empty($_FILES['new_picture']['name'])){
            ...
            $new_picture = mysqli_real_escape_string($dbc, trim(time() . $_FILES['new_picture']['name'])); //time() + new_picture's name
            $new_picture_type = $_FILES['new_picture']['type'];
            $new_picture_size = $_FILES['new_picture']['size']; 
            echo var_dump($_FILES['new_picture']['name']); 
            list($new_picture_width, $new_picture_height) = getimagesize($_FILES['new_picture']['tmp_name']); //http://php.net/manual/en/function.getimagesize.php Index 0 and 1 contains respectively the width and the height of the image.
    }
